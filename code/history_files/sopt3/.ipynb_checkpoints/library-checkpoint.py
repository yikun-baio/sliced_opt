...
def transpose(X):
    n=X.shape[0]
    XT=np.zeros((n,1),dtype=np.float64)
    for i in range(n):
        XT[i]=X[i]
    return XT
...
def cost_matrix_d(X,Y):
    ...
    n=X.shape[0]
    m=Y.shape[0]
    M=np.zeros((n,m),dtype=nb.float64)
    for i in range(n):
        for j in range(m):
...
@nb.njit([nb.types.Tuple((nb.int64,nb.float64))(nb.float64,nb.float64[:])],fastmath=True) 
#@nb.njit(fastmath=True)
def closest_y(x,Y):
    '''
    Parameters
...
#@nb.njit([nb.float64[:,:](nb.int64[:],nb.int64)],fastmath=True)
@nb.njit(fastmath=True)
...
@nb.njit([nb.types.Tuple((nb.float64,nb.int64[:],nb.float64,nb.int64[:]))(nb.int64,nb.float64)])
#@nb.njit()
...
# @nb.jit([nb.types.Tuple((nb.float64,nb.int64[:],nb.float64,nb.int64[:]))(nb.int64,nb.float64)],nopython=True)
...
@nb.jit([nb.types.Tuple((nb.float64,nb.int64[:],nb.float64,nb.int64[:]))(nb.float64[:,:],nb.int64,nb.int64,nb.float64)],nopython=True)
#@nb.njit()
...
@nb.njit([nb.types.Tuple((nb.float64,nb.int64[:],nb.float64,nb.int64[:]))(nb.float64[:,:],nb.int64,nb.int64,nb.float64)])
#@nb.njit()
def one_x_opt_np(M1,i_act:nb.int64,j_act:nb.int64,Lambda:nb.float64): 
    '''
...
def one_x_opt_T(M1: torch.Tensor,i_act:int,j_act:torch.Tensor,Lambda: torch.Tensor)->Tuple[torch.Tensor,torch.Tensor,torch.Tensor,torch.Tensor]:
    ...
    if c_xy>=Lambda:
        ...
        return c_xy,j_act.reshape(1),torch.tensor(0,device=device,dtype=torch.float64),torch.empty(0,device=device,dtype=torch.int64)
...
@nb.njit([nb.float64[:](nb.float64[:,:],nb.int64[:],nb.int64[:])],fastmath=True)
#@nb.njit(fastmath=True)
