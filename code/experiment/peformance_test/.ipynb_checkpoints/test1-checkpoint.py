...
def cost_matrix(vector_list):
    ...
    for i in range(N):
        for j in range(N):
            v1=vector_list[i]
            v2=vector_list[j]
            cost_M[i,j]=np.dot(v1,v2)/(np.linalg.norm(v1)*np.linalg.norm(v2))
    return cost_M
...
if status == pywraplp.Solver.OPTIMAL:
    ...
    for i in range(N):
        print('xi =', solution[i].solution_value())
