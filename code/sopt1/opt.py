...
def pot_1d(X,Y): 
    ...
    cost_xk_yjk=M[k,jk]    
    cost+=cost_xk_yjk
    L=np.concatenate((L,np.array([jk])))
...
#            L=np.append(L,jk)
            ...
            if j_act>=0:                        
                ...
                Lx=arange(0,k+1)
                cost2_list=matrix_take(M1,Lx,L1)
                cost2=np.sum(cost2_list)
            else:
                cost2=np.inf
            if cost1<cost2:
                cost=cost1
                L=np.concatenate((L,np.array([np.int32(j_last+1)])))
            elif cost2<=cost1:
                cost=cost2
                L=L1.copy()    
    return cost,L
