...
torch.save(kmeans,'PC/kmeans.pt')
...
Lambda=400
for epoch in range(nb_iter_max):
    loss,a,b=sliced_max_opt(k_centroids_source_torch,k_centroids_target_torch,Lambda,80)
    loss.backward()
    print('training Epoch {}/{}'.format(epoch+1, nb_iter_max))
    print('gradient is',torch.norm(k_centroids_source_torch.grad).item())
    print('-' * 10)
    with torch.no_grad():
        if torch.norm(k_centroids_source_torch.grad)>=10:
            lr1 =5e-1
        elif torch.norm(k_centroids_source_torch.grad)>=1:
            lr1 =5e-2
        else:
            lr1=1e-3            
        #print(grad.item())
