...
for n in range (start_n,end_n,step):
    ...
    for i in range (k):
        X=torch.rand(n,device=device)*(20-0)+0
  ...
  #      print(X)
    #    print(Y)
 #       
        M=cost_matrix_T(X,Y,device).to(device='cpu')
...
plt.legend(loc='best')
plt.show()
plt.plot(range(start_n,end_n,step),time1_list,label='ours v2')
plt.plot(range(start_n,end_n,step),time2_list,label='ours v2 np')
plt.plot(range(start_n,end_n,step),time3_list,label='ours v2 torch')
