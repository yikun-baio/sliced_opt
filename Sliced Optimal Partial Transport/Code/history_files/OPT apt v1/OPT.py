...
def one_point_OPT(X_sub,Y_sub,M_sub,j_active,Lambda):            
    if j_active<0:
        return 2*Lambda,[-numpy.inf],2*Lambda,[-numpy.inf]
    elif j_active>=0:
        c_xkyjk=M_sub[0,j_active] 
        if c_xkyjk>=2*Lambda:
            return 2*Lambda,[-numpy.inf],2*Lambda,[-numpy.inf]
        ...
        for case in case_set:
            ...
            #print('conflict')
                ...
                if cost_xk_yjlast<(2*Lambda):            
                    ...
                    Y1=Y[0:j_last]
                    M1=M[0:k,0:j_last]
                    cost_previous,L1=OPT_1D_v1(X1,Y1,M1,Lambda)
...
def OPT_sub(X_sub,Y_sub,L_sub,M_sub,Lambda):
    L_sub_previous=[]
    cost_sub_previous=0
    ...
    s=len(L_sub)
    if s==0:
        cost_sub,L_sub,cost_sub_previous,L_sub_previous=one_point_OPT(X_sub,Y_sub,M_sub,j_active,Lambda)
        return cost_sub,L_sub, cost_sub_previous,L_sub_previous
   
    
    s=M.shape[0] # it is the length of X_sub
    # separate the problem 
    Y_sub_assigned=Y_sub[L_sub] # It give the list y(L(i_start)),y(L(i_start+1)),....y(L(k-2)).
    L_x_sub_inactive=range(0,i_active)
...
#   cost_inactive=sum(cost_function(X_sub_inactive,Y_sub_inactive)) # no pair has distance exceeds 2Lambda
    #
    cost_inactive=sum(M[L_x_sub_inactive,L_sub_inactive]) # no pair has distance exceeds 2Lambda
    
#    X_sub_active=X_sub[i_active:]
#    Y_sub_active=Y_sub_assigned[i_active:]
    L_x_sub_active=range(i_active,s)
    ...
    if len(L_sub_active)==0:
        cost_sub,L_sub,cost_sub_previous,L_sub_previous=one_point_OPT(X_sub_active,Y_sub,M_sub,j_active,Lambda)
    ...
    for l in range(s,-1,i_active-1): # this is the index we extract x_jk,y_l-1 
        #xl1=X_sub_active[l]
        #yl=Y_sub_active[l-1]
        L_x_active_left=range()
        ...
        cost_left=sum((X_sub_active_left,Y_sub_active_left))+2*Lambda # it used to compute the previous cost and the total cost
        ...
        for case in case_set:
            if case=='0': # cost for destroy mass 
                ...
                if cost_xk_yjlast<2*Lambda and j_start<=m:
                    ...
                    cost_sub,L_sub,cost_sub_previous,L_sub_previous=OPT_sub(X_sub,Y_sub,L_sub_0,M,Lambda)
...
Y=numpy.random.uniform([2,10,6])+1
Lambda=3
M=cost_matrix(X,Y)
cost1,L1=OPT_1D_v1(X, Y, M,Lambda)

cost2,L2=OPT_1D_v3(X, Y, M,Lambda)
